...
/// RevenueCat customer info updates to AppMachina.
...
/// Optional RevenueCat integration for the AppMachina SDK.
///
...
/// and syncs subscriber status to AppMachina user properties.
///
/// Usage:
/// ```swift
/// // After initializing both AppMachina and RevenueCat:
/// AppMachina.shared.revenueCat.connect(Purchases.shared)
///
/// // Or manually track a purchase:
/// AppMachina.shared.revenueCat.trackPurchase(
...
public final class RevenueCatModule: @unchecked Sendable {
    ...
    private static let log = OSLog(subsystem: "com.appmachina.sdk", category: "RevenueCat")
    ...
    func attach(core: AppMachinaCoreHandle) {
        lock.lock()
        _core = core
        lock.unlock()
    }

    private var lockedCore: AppMachinaCoreHandle? {
        lock.lock()
        defer { lock.unlock() }
        return _core
    }
    ...
    /// Connect AppMachina to your existing RevenueCat Purchases instance.
    ///
    ...
    public func trackPurchase(
        ...
        do {
            try core.track(
                ...
                propertiesJson: AppMachina.jsonString(from: props),
                userId: nil,
                anonymousId: nil
            )
            return .success(())
        } catch {
            return .failure(AppMachina.mapError(error))
        }
    }
    ...
    private func syncUserProperties(isSubscriber: Bool, originalAppUserId: String) {
        ...
        do {
            try core.setUserProperties(
                propertiesJson: AppMachina.jsonString(from: userProps) ?? "{}"
            )
        } catch {
