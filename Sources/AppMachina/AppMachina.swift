...
// MARK: - AppMachinaError

/// All possible SDK errors. Mapped from Rust UniFfiError at the FFI boundary.
public enum AppMachinaError: Error, LocalizedError, Sendable, Equatable {
    case notInitialized
    ...
    public var errorDescription: String? {
        switch self {
        case .notInitialized:
            return "AppMachina SDK has not been initialized. Call AppMachina.initialize(config:) first."
        ...
        case .unknown(let reason):
            return "Unknown AppMachina SDK error: \(reason)"
        }
    }
}
...
typealias CoreError = AppMachinaError

// MARK: - Environment

// AppMachinaEnvironment is defined in Generated/AppMachinaCore.swift (UniFFI-generated).
// Named AppMachinaEnvironment (not Environment) to avoid collision with SwiftUI's @Environment.

// MARK: - AppMachinaConfig

public struct AppMachinaConfig: Sendable {
    ...
    /// Custom base URL for the ingest API. Defaults to the AppMachina production endpoint.
...
/// Main entry point for the AppMachina SDK.
...
public final class AppMachina: @unchecked Sendable, AppMachinaProtocol {

    // MARK: - Singleton

    public static let shared = AppMachina()
    ...
    private static let firstLaunchTrackedKey = "com.appmachina.firstLaunchTracked"
    ...
    ///
    /// ```swift
    /// AppMachina.shared.setInitListener { mainThreadMs, totalMs in
    ...
    private var _core: AppMachinaCoreHandle?
    ...
    private var _configEnvironment: AppMachinaEnvironment = .production
    ...
    /// `true` if the SDK had prior state (i.e. `com.appmachina.installId` already
    ...
    private let _monitorQueue = DispatchQueue(label: "com.appmachina.sdk.network-monitor")
    /// Serial queue for HTTP event delivery (drain → send → retry/requeue).
    private let _flushQueue = DispatchQueue(label: "com.appmachina.sdk.flush")
    ...
    private var core: AppMachinaCoreHandle? {
        lock.lock()
        defer { lock.unlock() }
        return _core
    }
    ...
    public func initialize(config: AppMachinaConfig) -> SafeResult<Void> {
        ...
        if config.appId.isEmpty {
            ...
            lock.unlock()
            let err = AppMachinaError.invalidConfig("appId must not be empty")
        ...
        do {
            handle = try AppMachinaCoreHandle.`init`(config: uniffiConfig)
    ...
    /// Track a typed event conforming to `AppMachinaEvent`.
    @discardableResult
    ...
    /// - Returns: `.success(())` on success, `.failure(AppMachinaError)` on failure.
    @discardableResult
    ...
    public func flushBlocking() -> SafeResult<Void> {
        ...
        if result == .timedOut {
            ...
            return .failure(AppMachinaError.networkError("flushBlocking timed out"))
        }
        return box.value
    }
    ...
    private func deliverBatch(core: AppMachinaCoreHandle) -> SafeResult<Void> {
        ...
        guard let url = URL(string: urlString) else {
            requeueSilently(core: core, batchJson: batchJson)
            let err = AppMachinaError.networkError("Invalid events URL: \(urlString)")
            reportError(method: "flush", error: err)
            return .failure(err)
        }
        ...
        guard Self.applyHeaders(from: headersJson, to: &request) else {
            requeueSilently(core: core, batchJson: batchJson)
            let err = AppMachinaError.networkError("Failed to parse flush headers")
            reportError(method: "flush", error: err)
            return .failure(err)
        }
        ...
        for attempt in 0..<Self.maxRetries {
            ...
            if let error = networkError {
                ...
                if status == 429 || (status >= 500 && status < 600) {
                    ...
                    let err = AppMachinaError.networkError("HTTP \(status)")
                    reportError(method: "flush", error: err)
                    return .failure(err)
                }
            }
        ...
        let err = AppMachinaError.networkError("All retries exhausted")
        reportError(method: "flush", error: err)
        return .failure(err)
    }

    /// Requeue a drained batch back into the Rust core queue. Errors are swallowed.
    private func requeueSilently(core: AppMachinaCoreHandle, batchJson: String) {
        do {
    ...
    public func debugStatus() -> String {
        ...
        var lines: [String] = []
        lines.append("AppMachina SDK Status")
    ...
    private func lockedCoreIfInitialized() -> AppMachinaCoreHandle? {
        lock.lock()
        defer { lock.unlock() }
    ...
    private func reportError(method: String, error: AppMachinaError) {
    ...
    static func mapError(_ error: Error) -> AppMachinaError {
    ...
    private static let log = OSLog(subsystem: "com.appmachina.sdk", category: "AppMachina")

    private static let installIdKey = "com.appmachina.installId"
    private static let attributionDeeplinkIdKey = "com.appmachina.deeplinkId"
    private static let attributionGclidKey = "com.appmachina.gclid"
    private static let attributionFbclidKey = "com.appmachina.fbclid"
    private static let attributionFbcKey = "com.appmachina.fbc"
    private static let attributionTtclidKey = "com.appmachina.ttclid"
    private static let attributionMsclkidKey = "com.appmachina.msclkid"
    ...
    ///    that — return `false`.
    /// 2. If the SDK had prior state (`com.appmachina.installId` already existed),
    ...
    func shouldTreatAsNewInstall(
        ...
        now: Date = Date(),
        appInstallDate: Date? = AppMachina.appInstallDate()
    ) -> Bool {
        ...
        guard let installDate = appInstallDate else {
            ...
            os_log("Install event gating: could not read app install date — trusting flag",
                   log: AppMachina.log, type: .info)
            return true
        }

        let elapsed = now.timeIntervalSince(installDate)
        let isRecentInstall = elapsed <= AppMachina.installEventMaxDiffSecs
        ...
        if !isRecentInstall && enableDebug {
            os_log(
                ...
                log: AppMachina.log, type: .debug,
                elapsed, AppMachina.installEventMaxDiffSecs
            )
        }
    ...
    private func trackAttributionSignals(core: AppMachinaCoreHandle, clipboardAttributionEnabled: Bool, autoTrackAppOpen: Bool) {
    ...
    private static let defaultBaseUrl = "https://in.appmachina.com"
    ...
    private static func sdkVersionString() -> String {
        if let version = Bundle(for: AppMachina.self).infoDictionary?["CFBundleShortVersionString"] as? String {
            return version
        }
        return "unknown"
    }
    ...
    private static func persistenceDirectory() -> String {
        ...
        guard let baseUrl = urls.first else {
            ...
            let fallback = NSTemporaryDirectory() + "com.appmachina.sdk"
            do {
        ...
        let dir = baseUrl.appendingPathComponent("com.appmachina.sdk", isDirectory: true)
        do {
...
#if canImport(UIKit) && !os(watchOS)
@available(iOS 14.0, macOS 12.0, tvOS 14.0, watchOS 7.0, *)
public extension AppMachina {
    /// Show the debug overlay on the given window. Delegates to ``AppMachina/showDebugOverlay(in:)``.
    static func showDebugOverlay(in window: UIWindow) {
        shared.showDebugOverlay(in: window)
    }

    /// Hide the debug overlay. Delegates to ``AppMachina/hideDebugOverlay()``.
...
@available(iOS 14.0, macOS 12.0, tvOS 14.0, watchOS 7.0, *)
public extension AppMachina {
    /// Initialize the shared AppMachina SDK instance. Delegates to ``AppMachina/initialize(config:)``.
    @discardableResult
    static func initialize(config: AppMachinaConfig) -> SafeResult<Void> {
        shared.initialize(config: config)
    }

    /// Track a custom event with optional properties. Delegates to ``AppMachina/track(_:properties:)``.
    @discardableResult
    static func track(_ event: String, properties: [String: Any] = [:]) -> SafeResult<Void> {
        shared.track(event, properties: properties)
    }

    /// Track a typed event. Delegates to ``AppMachina/track(_:)-7k2x3``.
    @discardableResult
    static func track(_ event: some AppMachinaEvent) -> SafeResult<Void> {
        shared.track(event)
    }

    /// Record a screen view with optional properties. Delegates to ``AppMachina/screen(_:properties:)``.
    @discardableResult
    static func screen(_ name: String, properties: [String: Any] = [:]) -> SafeResult<Void> {
        shared.screen(name, properties: properties)
    }

    /// Identify a user and optionally set traits. Delegates to ``AppMachina/identify(userId:traits:)``.
    @discardableResult
    static func identify(userId: String, traits: [String: Any] = [:]) -> SafeResult<Void> {
        shared.identify(userId: userId, traits: traits)
    }

    /// Associate all subsequent events with a group. Delegates to ``AppMachina/group(groupId:properties:)``.
    @discardableResult
    static func group(groupId: String, properties: [String: Any] = [:]) -> SafeResult<Void> {
        shared.group(groupId: groupId, properties: properties)
    }

    /// Set the app-scoped user ID for attribution. Delegates to ``AppMachina/setAppUserId(_:)``.
    @discardableResult
    static func setAppUserId(_ userId: String) -> SafeResult<Void> {
        shared.setAppUserId(userId)
    }

    /// Clear the current app-scoped user ID. Delegates to ``AppMachina/clearAppUserId()``.
    @discardableResult
    static func clearAppUserId() -> SafeResult<Void> {
        shared.clearAppUserId()
    }

    /// Set custom user properties on the current user. Delegates to ``AppMachina/setUserProperties(_:)``.
    @discardableResult
    static func setUserProperties(_ properties: [String: Any]) -> SafeResult<Void> {
        shared.setUserProperties(properties)
    }

    /// Set user properties with "set once" semantics. Delegates to ``AppMachina/setUserPropertiesOnce(_:)``.
    @discardableResult
    static func setUserPropertiesOnce(_ properties: [String: Any]) -> SafeResult<Void> {
        shared.setUserPropertiesOnce(properties)
    }

    /// Update consent settings for analytics and advertising. Delegates to ``AppMachina/setConsent(_:)``.
    @discardableResult
    static func setConsent(_ consent: ConsentSettings) -> SafeResult<Void> {
        shared.setConsent(consent)
    }

    /// Store attribution data for server-side matching. Delegates to ``AppMachina/setAttributionData(deeplinkId:gclid:fbclid:ttclid:msclkid:)``.
    @discardableResult
    ...
    /// Flush queued events to the server asynchronously. Delegates to ``AppMachina/flush()``.
    static func flush() async -> SafeResult<Void> {
        await shared.flush()
    }

    /// Flush queued events synchronously, blocking the calling thread. Delegates to ``AppMachina/flushBlocking()``.
    /// - Warning: Do not call from async contexts or the main actor. See ``AppMachina/flushBlocking()``.
    @discardableResult
    static func flushBlocking() -> SafeResult<Void> {
        shared.flushBlocking()
    }

    /// Reset the SDK state, clearing user identity and properties. Delegates to ``AppMachina/reset()``.
    @discardableResult
    static func reset() -> SafeResult<Void> {
        shared.reset()
    }

    /// Shut down the SDK and release resources. Delegates to ``AppMachina/shutdown()``.
    @discardableResult
    static func shutdown() -> SafeResult<Void> {
        shared.shutdown()
    }

    /// Set a listener to receive SDK initialization timing metrics. Delegates to ``AppMachina/setInitListener(_:)``.
    static func setInitListener(_ listener: ((Double, Double) -> Void)?) {
        shared.setInitListener(listener)
    }

    /// Returns a formatted string describing the current SDK state. Delegates to ``AppMachina/debugStatus()``.
