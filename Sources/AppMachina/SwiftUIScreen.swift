...
/// A `ViewModifier` that emits an AppMachina `$screen_view` event whenever
/// the modified view appears. Use via the `View.appMachinaScreen(name:screenClass:)`
...
public struct AppMachinaScreenModifier: ViewModifier {
    public let name: String
    public let screenClass: String?

    /// Test hook — when set, `onAppear` calls this closure instead of
    /// `AppMachina.shared.screen(...)`. Production callers leave it `nil`.
    ...
    public func body(content: Content) -> some View {
        content.onAppear {
            ...
            if let hook = testHook {
                ...
                _ = AppMachina.shared.screen(name, properties: props)
            }
        }
    }
}
...
public extension View {
    /// Emit an AppMachina `$screen_view` whenever this view appears.
    ///
    /// ```swift
    /// var body: some View {
    ///     ProductDetailView(product: product)
    ///         .appMachinaScreen(name: "Product Detail", screenClass: "ProductDetailView")
    /// }
    /// ```
    ///
    /// - Parameters:
    ...
    func appMachinaScreen(name: String, screenClass: String? = nil) -> some View {
        modifier(AppMachinaScreenModifier(name: name, screenClass: screenClass))
    }
}
#endif
