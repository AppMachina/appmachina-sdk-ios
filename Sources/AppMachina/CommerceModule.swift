...
public final class CommerceModule: @unchecked Sendable {
    ...
    func attach(core: AppMachinaCoreHandle, skan: SKANModule) {
        lock.lock()
        _core = core
        _skan = skan
        lock.unlock()
    ...
    ///
    /// **Opt-in.** Wired by `AppMachina.initialize` only when
    /// `AppMachinaConfig(automaticPurchaseTrackingEnabled: true)` is passed —
