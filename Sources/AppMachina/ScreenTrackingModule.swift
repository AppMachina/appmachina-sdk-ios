...
/// SwiftUI consumers should use the `View.appMachinaScreen(name:screenClass:)`
/// modifier instead.
///
/// Opt out via `AppMachinaConfig(automaticScreenTrackingEnabled: false)`.
...
public final class ScreenTrackingModule: @unchecked Sendable {
    ...
    /// without a fully initialized `AppMachina` singleton.
    ...
    private static let log = OSLog(subsystem: "com.appmachina.sdk", category: "screen-tracking")
    ...
    /// Attach to the AppMachina singleton.
    ...
    static func swizzleViewDidAppearIfNeeded() {
        ...
        UIViewController.appmachina_installViewDidAppearSwizzle()
...
extension UIViewController {
    ...
    @objc fileprivate static func appmachina_installViewDidAppearSwizzle() {
        let originalSelector = #selector(UIViewController.viewDidAppear(_:))
        let swizzledSelector = #selector(UIViewController.appmachina_swizzled_viewDidAppear(_:))
    ...
    @objc fileprivate func appmachina_swizzled_viewDidAppear(_ animated: Bool) {
        // Because `method_exchangeImplementations` swapped the IMPs, this call
        // resolves to the *original* `viewDidAppear(_:)`.
        appmachina_swizzled_viewDidAppear(animated)
