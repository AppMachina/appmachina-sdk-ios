...
///
/// All persisted state lives in `UserDefaults` under the `appmachina.*` namespace.
/// The persisted keys are intentionally separate from the legacy `com.appmachina.*`
/// keys so the new namespace doesn't interact with existing install gating.
///
/// Opt out via `AppMachinaConfig(automaticLifecycleTrackingEnabled: false)`.
...
public final class LifecycleModule: @unchecked Sendable {
    ...
    public static let firstOpenEmittedKey = "appmachina.first_open_emitted"

    /// `Bool` — set to `true` after the first `$app_install` is emitted.
    public static let appInstallEmittedKey = "appmachina.app_install_emitted"

    /// `String` — last seen `CFBundleVersion`. Compared on attach to detect `$app_update`.
    public static let lastBundleVersionKey = "appmachina.last_bundle_version"

    // MARK: - Tracker Abstraction

    /// Closure-based tracker injection. Exists so unit tests can run without
    /// a fully initialized `AppMachina` singleton (which requires the Rust core
    ...
    private static let log = OSLog(subsystem: "com.appmachina.sdk", category: "lifecycle")
    ...
    /// Attach to the AppMachina singleton. Production entry point.
